...
from MBRL.distance import wasserstein
class MBRL_net(object):
    ...
    def _build_graph(self, x, t, y_ , p_t, z_norm, FLAGS, r_lambda, do_in, do_out, dims):
        ...
        with tf.variable_scope('epsilon_t') as scope:
            epsilon_t = tf.Variable(tf.random_normal([]), name='epsilon_t')
        with tf.variable_scope('epsilon_y') as scope:
            epsilon_y = tf.Variable(tf.random_normal([]), name='epsilon_y')
        ...
        rep_loss = FLAGS.p_alpha * imb_dist
        '''Adversarial'''
        ...
        if FLAGS.reweight_sample_t==1:
            ...
            discriminator_loss = -tf.reduce_mean(sample_weight_t * (t * tf.log(tpre) + (1.0 - t) * tf.log(1.0 - tpre))) + FLAGS.p_epsilon_t * epsilon_t * tf.abs(tf.reduce_mean(t-tpre))
        else:
            discriminator_loss = -tf.reduce_mean(t * tf.log(tpre) + (1.0 - t) * tf.log(1.0 - tpre)) + FLAGS.p_epsilon_t * epsilon_t * tf.abs(tf.reduce_mean(t-tpre))
        ...
        ''' Total error '''
        tot_error = risk + FLAGS.p_epsilon_y * epsilon_y * tf.abs(tf.reduce_mean(y_-y))
