...
def table_str_bin(result_set, row_labels, labels_long=None, binary=False):
    if binary:
        cols = ['pehe', 'bias_ate','bias_ate_dml','bias_ate_rcl_1','bias_ate_rcl_2', 'rmse_fact', 'objective', 'auc', 'fact_auc']
    else:
        cols = ['pehe', 'bias_ate','bias_ate_dml','bias_ate_rcl_1','bias_ate_rcl_2', 'rmse_fact', 'objective']
...
def plot_evaluation_cont(results, configs, output_dir, data_train_path, data_test_path, filters=None, epsilon=0, early_stop_criterion='rmse_fact'):
    EARLY_STOP_CRITERION_CONT = early_stop_criterion
