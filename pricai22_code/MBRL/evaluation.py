...
import MBRL.compute_theta as compute_theta


from MBRL.logger import Logger as Log
from MBRL.loader import *
...
def evaluate_bin_att(predictions, tpre, data, i_exp, epsilon, I_subset=None,
    ...
    try:
        ...
        xi = yf - yf_p
        # xi_abs_mean = np.abs(np.mean(xi))
        nu = t - tpre
        # nu_abs_mean = np.abs(np.mean(nu))
        xinu_abs_mean = np.abs(np.mean(xi.reshape(-1, 1) * nu.reshape(-1, 1)))

        return {'ate_pred': ate_pred, 'att_pred': att_pred, 'ate_abs': np.abs(bias_ate), 'purt_rmse_fact': rmse_fact + epsilon*(xinu_abs_mean), 
                'bias_ate_dml':np.nan, 'bias_ate_rcl_1':np.nan, 'bias_ate_rcl_2':np.nan,
                'atc_pred': atc_pred, 'bias_ate': bias_ate,
                'bias_att': bias_att, 'bias_atc': bias_atc,
                'rmse_fact': rmse_fact, 'rmse_cfact': rmse_cfact,
                'pehe': pehe, 'rmse_ite': rmse_ite, 'pehe_nn': pehe_appr, 'auc': auc,'roc_auc':roc_auc, 'purt_fact_auc': auc - epsilon*(xinu_abs_mean),
                'fact_auc':fact_auc,'fact_roc_auc':fact_roc_auc }




def evaluate_cont_ate(predictions, tpre, data, i_exp, epsilon, I_subset=None,
    compute_policy_curve=False, nn_t=None, nn_c=None):
    ...
    return {'ate_pred': ate_pred, 'att_pred': att_pred, 'ate_abs': np.abs(bias_ate), 'purt_rmse_fact': rmse_fact + epsilon*(xinu_abs_mean),
            ...
            'bias_att': bias_att, 'bias_atc': bias_atc,
            'rmse_fact': rmse_fact, 'rmse_cfact': rmse_cfact,
            'pehe': pehe, 'rmse_ite': rmse_ite}

def evaluate_result(result, data, validation=False,
        multiple_exps=False, binary=False, epsilon=0):
    ...
    for i_out in range(n_outputs):
        ...
        for i_rep in range(n_rep):
            ...
            if binary:
                eval_result = evaluate_bin_att(predictions[:,:,i_rep,i_out], tpre[:,:,i_rep,i_out],
                    data, i_exp, I_valid_rep, epsilon, compute_policy_curve, nn_t=nn_t, nn_c=nn_c)
            else:
                eval_result = evaluate_cont_ate(predictions[:,:,i_rep,i_out], tpre[:,:,i_rep,i_out],
                    data, i_exp, epsilon, I_valid_rep, compute_policy_curve, nn_t=nn_t, nn_c=nn_c)
...
def evaluate(output_dir, data_path_train, data_path_test=None, binary=False, epsilon=0):
    ...
    for result in results:
        ...
        try:           
            ...
            eval_valid = evaluate_result(result['train'], data_train,
                validation=True, multiple_exps=multiple_exps, binary=binary, epsilon=epsilon)

            if data_test is not None:
                eval_test = evaluate_result(result['test'], data_test,
                    validation=False, multiple_exps=multiple_exps, binary=binary, epsilon=epsilon)
            else:
                eval_test = None
...
def eval_ate(result, data_train, data_test, i_exp, i_sel):
    ate_dml_train, ate_rcl_train_1, ate_rcl_train_2 = compute_ate(result['train'], data_train, i_exp, i_sel, validation=False)
    ate_dml_valid, ate_rcl_valid_1, ate_rcl_valid_2 = compute_ate(result['train'], data_train, i_exp, i_sel, validation=True)
    ate_dml_test, ate_rcl_test_1, ate_rcl_test_2 = compute_ate(result['test'], data_test, i_exp, i_sel, validation=False)
    return ate_dml_train, ate_rcl_train_1, ate_rcl_train_2, ate_dml_valid, ate_rcl_valid_1, ate_rcl_valid_2, ate_dml_test, ate_rcl_test_1, ate_rcl_test_2
def compute_ate(result, data, i_exp, i_sel, validation):
    ...
    theta1_rcl_1 = compute_theta.compute_rcl_theta(y1_pre, res_m, y1_minus_g1_given_z_list, 2, 1, E_res_m_list)
    theta0_rcl_1 = compute_theta.compute_rcl_theta(y0_pre, res_m, y0_minus_g0_given_z_list, 2, 1, E_res_m_list)
    theta1_rcl_2 = compute_theta.compute_rcl_theta(y1_pre, res_m, y1_minus_g1_given_z_list, 2, 2, E_res_m_list)
    theta0_rcl_2 = compute_theta.compute_rcl_theta(y0_pre, res_m, y0_minus_g0_given_z_list, 2, 2, E_res_m_list)
    ate_rcl_1 = theta1_rcl_1 - theta0_rcl_1
    ate_rcl_2 = theta1_rcl_2 - theta0_rcl_2
    true_ate = np.mean(mu1-mu0)
    bias_ate_dml = true_ate - ate_dml
    bias_ate_rcl_1 = true_ate - ate_rcl_1
    bias_ate_rcl_2 = true_ate - ate_rcl_2
    return bias_ate_dml, bias_ate_rcl_1, bias_ate_rcl_2